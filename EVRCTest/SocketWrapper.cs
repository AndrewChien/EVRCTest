...
namespace EVRCTest
{
    public class SocketWrapper
    {
        ...
        /// <returns></returns>
        public int socket_receive(ref byte[] readBuffer)
        {
            try
            {
                if (theSocket == null)
        ...
        /// <returns></returns>
        public bool socket_send(byte[] sendMessage)
        {
            if (checkSocketState())
            {
        ...
        /// <returns></returns>
        private static bool SendData(byte[] data)
        {
            bool result = false;
            ...
            try
            {
                //byte[] cmd = Encoding.Default.GetBytes(data);
                int n = theSocket.Send(data);
                if (n < 1)
                    result = false;
            }
            catch (Exception ee)
